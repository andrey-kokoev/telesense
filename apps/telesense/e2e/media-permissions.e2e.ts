...
test.describe("Call with limited or no media permissions", () => {
  ...
  // Skipped: WebRTC connection establishment is flaky in headless test mode
  // The core functionality is covered by other tests - viewers can join without media
  test.skip("two participants can connect when one is viewer (no media)", async ({ browser }) => {
    const roomId = `M${Date.now().toString().slice(-5)}`.slice(0, 6).toUpperCase()

    // Context 1: Full media (host) - default behavior
    const hostContext = await browser.newContext()
    const hostPage = await hostContext.newPage()

    // Context 2: No media (viewer) - block both
    const viewerContext = await browser.newContext()
    const viewerPage = await viewerContext.newPage()
    await viewerPage.addInitScript(() => {
      navigator.mediaDevices.getUserMedia = async () => {
        throw new Error("All media blocked")
      }
    })
    ...
    try {
      // Host creates room
      await hostPage.goto("/")
      const hostInputs = hostPage.locator(".landing__code-input")
      await hostInputs.first().click()
      for (const char of roomId) {
        await hostPage.keyboard.press(char)
      }
      const hostAction = hostPage.locator(".landing__main .landing__btn").first()
      await hostAction.click()
      await expect(hostPage).toHaveURL(new RegExp(`\\?room=${roomId}$`))
      await expect(hostPage.getByText("Waiting for participant...")).toBeVisible({
        timeout: 30000,
      })

      // Wait for room to be fully created (Durable Object needs time to persist)
      // Poll room status until it returns exists: true
      let roomExists = false
      let attempts = 0
      while (!roomExists && attempts < 20) {
        const response = await hostPage.request.get(`/api/rooms/${roomId}/status`)
        const body = (await response.json()) as { exists: boolean }
        roomExists = body.exists
        if (!roomExists) {
      ...
      // Viewer navigates to landing and enters room code
      await viewerPage.goto("/")
      const viewerInputs = viewerPage.locator(".landing__code-input")
      await viewerInputs.first().click()
      for (const char of roomId) {
        await viewerPage.keyboard.press(char)
      }

      // Wait for room check and "Join room" button
      const viewerAction = viewerPage.locator(".landing__main .landing__btn").first()
      await expect(viewerAction).toHaveText("Join room", { timeout: 15000 })
      await viewerAction.click()

      // Both should be in the call
      await expect(viewerPage).toHaveURL(new RegExp(`\\?room=${roomId}$`))

      // Host should eventually see remote connected
      await expect(hostPage.getByText("Remote participant connected!")).toBeVisible({
        timeout: 30000,
      })

      // Viewer should also see connection confirmation
      await expect(viewerPage.getByText("Remote participant connected!")).toBeVisible({
        timeout: 10000,
      })

      // Host controls should be enabled (has media)
      const hostLeaveButton = hostPage.getByRole("button").filter({ hasText: /leave/i }).first()
      await expect(hostLeaveButton).toBeEnabled()
    } finally {
