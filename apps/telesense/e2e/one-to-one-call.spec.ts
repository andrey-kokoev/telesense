...
test.describe("1:1 Video Call", () => {
  ...
  test("caller joins and publishes tracks", async () => {
    ...
    await expect(callerPage.locator("#status")).toContainText("Local media captured", {
      timeout: 15000,
    })

    // Wait for session creation
    await expect(callerPage.locator("#status")).toContainText("Session created", {
      timeout: 15000,
    })

    // Wait for publish
    await expect(callerPage.locator("#status")).toContainText("Published and connected", {
      timeout: 20000,
    })

    // Wait for ICE connected
    await expect(callerPage.locator("#status")).toContainText("Publish ICE connected", {
      timeout: 20000,
    })
  ...
  test("callee joins and both see each other", async () => {
    ...
    await expect(calleePage.locator("#status")).toContainText("Published and connected", {
      timeout: 20000,
    })
    ...
    await expect(callerPage.locator("#status")).toContainText("new remote track", {
      timeout: 15000,
    })
    ...
    await expect(callerPage.locator("#status")).toContainText("Subscribed to remote tracks", {
      timeout: 20000,
    })
    await expect(calleePage.locator("#status")).toContainText("Subscribed to remote tracks", {
      timeout: 20000,
    })
